#include "nrf24l01.h"
#include "nrf24l01_low_level.c"

uint8_t read24_return;
...
uint8_t nrf24_receive(uint8_t *payload, uint8_t payload_width)
{
...

  {
    ...
    if (register_current_value & (1 << RX_DR))                         /*if received data is ready inside RX FIFO*/
    {
      ...
      debug = SPI_send_command(SPI_command);
  ...
  //        printf("Received Payload: 0x%08x.\n", *payload);
        payload++;
      }
...
void nrf24_reset()
{
  ...
  nrf24_crc_configuration(DISABLE, 1);
...
void nrf24_device(uint8_t device_mode, uint8_t reset_state)
{
  ...
  printf("Setting Device Mode Now\n");
  switch (device_mode)
  {
    case TRANSMITTER:
    printf("TRANSMITTER\n");
...
//  printf("\nNRF24 READ\n\n");
  ...
  read24_return = SPI_send_command(SPI_command);
//    printf("NRF24_READ RETURN: 0x%02x.\n", read24_return);
  SPI_command = NOP_CMD;
...
void nrf24_write(uint8_t address, uint8_t *value, uint8_t data_length, uint8_t spi_state)
{
  ...
  for (; data_length ; data_length--)
  {
    ...
    value++;
    read24_return = SPI_send_command(SPI_command);
//      printf("Return of NRF24_WRITE: 0x%02x.\n", SPI_command);
  }
