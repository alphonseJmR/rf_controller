...
#define rot_dma_
// #define hall_dma_
#define rf_controller_dma
#define controller_structs
//  these are our general structs
...
// we'll also assume our read
// rep = rotary encoder program
bool rep_dma_fire(repeating_timer_t *rt){
//printf("timer\n");
  if(dma_channel_is_busy(controller_dma.channel)){
  //  printf("DMA Busy\n");
  // printf("Write to address: 0x%08x.\n", &dma_input.reg_ret);
  }else{
    dma_channel_set_read_addr(controller_dma.channel, &pio0->rxf[controller_rf.info.re_SM], false);
    dma_channel_set_write_addr(controller_dma.channel, &controller_rf.reg_return, false);
    dma_channel_set_trans_count(controller_dma.channel, 1, true);
    dma_channel_start(controller_dma.channel);
  }
}
// setup rotary encoder program
bool setup_rep_dma_timer(repeating_timer_t *rt, uint16_t delay){
  if(add_repeating_timer_us(delay, rep_dma_fire, NULL, &dma_c2)){
    printf("Rotary Encoder Program Dma Timer Setup.\n");
    return true;
  }
  return false;
}
...
bool setup_rep_dma(reg_dma_info *rdma, reir *result, repeating_timer_t *rt, uint16_t delay){
  ...
  channel_config_set_dreq(&c, pio_get_dreq(result->info.re_port, result->info.re_SM, false));
    printf("\nWrite to address: 0x%08x.\n", &result->reg_return);
    printf("Read from address: 0x%08x.\n\n", (result->info.re_port == pio0) ? &pio0->rxf[result->info.re_SM] : &pio1->rxf[result->info.re_SM]);
  dma_channel_configure(rdma->channel, &c, &result->reg_return, ((result->info.re_port == pio0) ? &pio0->rxf[result->info.re_SM] : &pio1->rxf[result->info.re_SM]), 1, false);
  return setup_rep_dma_timer(rt, delay);
}
#endif
#ifdef hall_dma_
