...
bool rep_dma_fire(repeating_timer_t *rt){
  ...
  // printf("Write to address: 0x%08x.\n", &dma_input.reg_ret);
    ...
    dma_channel_wait_for_finish_blocking(controller_dma.channel);
  }
printf("Reg Result: 0x%04x.\n", controller_rf.reg_return);
...
//  channel_config_set_transfer_data_size(&c, DMA_SIZE_8);
  ...
  dma_channel_configure(rdma->channel, &c, &result->reg_return, ((result->info.re_port == pio0) ? &pio0->rxf[result->info.re_SM] : &pio1->rxf[result->info.re_SM]), DMA_SIZE_32, true);
  channel_config_set_chain_to(&c, rdma->channel);
